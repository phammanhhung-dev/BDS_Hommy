...
async function updateAllArticles() {
  console.log("🚀 Bắt đầu cập nhật nội dung phong phú cho 14 bài viết...");
  try {
    for (const post of RICH_ARTICLES) {
      const [result] = await db.execute(
        `UPDATE baiviet 
         SET TieuDe = ?, TomTat = ?, NoiDung = ?, HinhAnh = ?
         WHERE BaiVietID = ?`,
        [post.TieuDe, post.TomTat, post.NoiDung.trim(), post.HinhAnh, post.BaiVietID]
      );
      console.log(`✅ Đã cập nhật Bài viết ID ${post.BaiVietID}: "${post.TieuDe}" (affected: ${result.affectedRows})`);
    }

    // Kiểm tra lại độ dài sau khi update
    const [checkRows] = await db.query(
      `SELECT BaiVietID, TieuDe, LENGTH(NoiDung) as content_length, Loai FROM baiviet ORDER BY BaiVietID ASC`
    );
    console.log("\n📊 Thống kê độ dài nội dung sau khi cập nhật:");
    console.table(checkRows);
    console.log("\n🎉 Hoàn thành cập nhật database thành công!");
  } catch (error) {
